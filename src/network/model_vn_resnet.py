...
class FcBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('x shape after prep1 : ', x.shape)  #[[1024, 128, 7]
        ...
        x = self.fc1(x)
        # print('x shape after fc1 : ', x.shape)  #[1024, 512]
        ...
        # print('x shape after fc3 : ', x.shape)  #[1024, 3]
        return x
...
class VN_ResNet1D(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('shape of x before local_pool : ', x.shape)  #[1024, 128, 25]  -> [1024, 21, 6, 25]
        # x = self.local_pool(x,2)      
        b,c,h,w = x.shape
        x = self.local_pool(x.reshape(-1, h,w))  
        x = x.reshape(b,c,x.shape[1],x.shape[2])   
        
        # print('shape of x after local_pool : ', x.shape)  #[1024, 128, 25]  -> [1024, 21, 6, 25]
        ...
        # print('rotated value x after encoder : ', x[:1,:1,:,0]) 
        # # <<< SO(3) Equivariance Check
